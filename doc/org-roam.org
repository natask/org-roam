...
Org-roam caches all Org links within its database. These include:

- ~file:~ links :: These regular file links are the most common way to link
  between zettels, and are native to Org.
- ~id:~ links :: It may sometimes be desirable to link to a particular headline.
  Org-roam enforces the use of Org IDs with headline linking, and stores these
  IDs within its database.

Links can be inserted with the title as the link description with
~org-roam-insert~.
...
In addition, Org-roam introduces a new kind of link: the ~roam:~ link. These
links are not natively supported by Org. This means that ~roam:~ links are only
navigable with Org-roam installed. These links provide a convenient way of
linking without breaking the flow of typing.

~roam:~ links come in three forms:

- ~[[roam:title]]~ :: this links to the note with title "title" in the database.
- ~[[roam:*headline]]~ :: this links to headline "headline" in the current Org-roam file.
- ~[[roam:title*headline]]~ :: this links to headline "headline" in the Org-roam file with title "title".

Org-roam provides completions for conveniently linking to existing notes. These completions are provided via ~completion-at-point~. Users of ~company-mode~ may want to prepend ~company-capf~ to the start of their ~company-backends~ for Org-mode.

- User Option: org-roam-link-auto-replace

  When non-nil, replace Org-roam's roam links with file or id links whenever
  possible. This includes on file-save, and on following the link. This defaults
  to ~t~. This behaviour is desirable to maintain the highest backwards
  compatibility with vanilla Org-mode, but the resolved links are harder to
  alter after resolution. Link resolution only occurs if there is a
  corresponding file or headline found.
